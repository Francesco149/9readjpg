/*
    standalone port of 9front's readjpg
    http://9front.org/9front/sys/src/cmd/jpg/readjpg.c

    LICENSE: http://9front.org/9front/lib/legal/NOTICE
*/

/* TODO: refactor to my coding style? */
...
static
void
errstr_s(char* buf, size_t bufsize)
{
...
jpgerror(Header *h, char *fmt, ...)
{
    ...
    va_start(arg, fmt);
    vsprintf(h->err, fmt, arg);
    va_end(arg);
    if(h->image != 0)
        fprintf(stderr, "jpg: partial image: %s\n", h->err);
    werrstr("%s", h->err);
    longjmp(h->errlab, 1);
}

Rawimage**
readjpg(FILE* fd, int colorspace)
{
    Rawimage *r, **array;
    Header *h;
    char buf[ERRMAX];
    ...
    if(colorspace!=CYCbCr && colorspace!=CRGB){
        errstr_s(buf, sizeof buf);    /* throw it away */
    ...
    while(header->err[0] == '\0'){
        ...
        case APPn+0:
            if(nseg==1 && strncmp((char*)b, "JFIF", 4)==0)  /* JFIF header; check version */
                if(b[5]>1 || b[6]>2)
                    sprintf(header->err, "ReadJPG: can't handle JFIF version %d.%2d", b[5], b[6]);
            break;
        ...
        case  SOS:
            ...
            case SOF2:
                ...
                break;
            default:
                sprintf(header->err, "unrecognized or unspecified encoding %d", header->mode);
                break;
            }
            break;
        ...
        default:
            sprintf(header->err, "ReadJPG: unknown marker %.2x", m);
            break;
        }
    }
    return image;
...
restart(Header *h, int mcu)
{
    ...
    if(nskip != 2)
        sprintf(h->err, "ReadJPG: skipped %d bytes at restart %d\n", nskip-2, rest);
    if(rst < 0)
        jpgerror(h, readerr);
    if((rst&7) != (rest&7))
